...
func (m *mysqlRoomsRepository) FetchAll(ctx context.Context) (res []domain.Rooms, err error) {
	...
	err = m.Db.WithContext(ctx).Model(&domain.Rooms{}).Preload("RoomTypes").Preload("RoomLocations").Find(&rooms).Error
...
func (m *mysqlRoomsRepository) FetchByID(ctx context.Context, id int) (res domain.Rooms, err error) {
	...
	err = m.Db.WithContext(ctx).Model(&domain.Rooms{}).Preload("RoomTypes").Preload("RoomLocations").Where("id = ?", id).First(&room).Error
	return room, err
}

func (m *mysqlRoomsRepository) Store(ctx context.Context, r *domain.Rooms) error {
	err := m.Db.WithContext(ctx).Model(&domain.Rooms{}).Preload("RoomTypes").Preload("RoomLocations").Create(&r).Error

	return err
}

func (m *mysqlRoomsRepository) Update(ctx context.Context, r *domain.Rooms, id int) error {
	err := m.Db.WithContext(ctx).Model(&domain.Rooms{}).Preload("RoomTypes").Preload("RoomLocations").Where("id = ?", id).Updates(&r).Error

	return err
}

func (m *mysqlRoomsRepository) Delete(ctx context.Context, id int) error {
	err := m.Db.WithContext(ctx).Preload("RoomTypes").Preload("RoomLocations").Where("id = ?", id).Delete(&domain.Rooms{}).Error
	return err
}
